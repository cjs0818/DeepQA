...
class Chatbot:
    ...
    def parseArgs(args):
        ...
        datasetArgs.add_argument('--corpus', choices=['fbdata', 'cornell', 'opensubs', 'lightweight'], default='fbdata', help='corpus on which extract the dataset. Only two corpus available right now (cornell and opensubs)')  # TODO: Replace by cst registered in textdata
    ...
    def loadModelParams(self):
        ...
        if not self.args.reset and not self.args.createDataset and os.path.exists(configName):
            ...
            self.args.vocabLimit = config['General'].getint('vocabLimit')
            ...
            print('vocabLimit: {}'.format(self.args.vocabLimit))
    ...
    def saveModelParams(self):
        ...
        config['General']['vocabLimit'] = str(self.args.vocabLimit)
